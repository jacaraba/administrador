	...
	*/
	...
	function get_sql_from($table_name, $skip_permissions = false, $skip_joins = false, $lower_permissions = false) {
		$sql_from = [
			...
			'amigos' => "`amigos` LEFT JOIN `lideres` as lideres1 ON `lideres1`.`CEDULA`=`amigos`.`CEDULA` LEFT JOIN `divpol2022` as divpol20221 ON `divpol20221`.`PUESTO`=`amigos`.`PUESTO` ",
